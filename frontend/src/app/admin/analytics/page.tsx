...
function Analytics() {
  ...
  const statCards = overallStats
    ? [
        { title: 'Total Revenue', value: formatMoney(overallStats.revenue.total), icon: DollarSign },
        { title: 'Total Orders', value: overallStats.orders.total.toLocaleString(), icon: ShoppingBag },
        { title: 'New Customers', value: (newCustomers ?? 0).toLocaleString(), icon: Users, hint: 'this month' },
        {
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      <AdminHeader title="Analytics & Reports" description="Track your business performance">
        ...
        <Button variant="outline" onClick={handleExport} disabled={!sales || sales.chart.length === 0}>
                ...
                <Card key={stat.title}>
                  <CardContent className="p-6">
                    ...
                    <div>
                      ...
                      {stat.hint && <p className="text-sm text-muted-foreground mt-1">{stat.hint}</p>}
                    </div>
                  </CardContent>
                </Card>
              ))}
                      ...
                      <div key={item.product.id} className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <div className="h-8 w-8 rounded-full bg-accent-rose/10 flex items-center justify-center shrink-0">
                            <span className="text-sm font-semibold text-accent-rose">{index + 1}</span>
                          </div>
                          <div>
                      ...
                      <div key={category.categoryId}>
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-3">
                            ...
                            <p className="text-sm text-muted-foreground">{category.itemsSold} items</p>
                          </div>
                          <p className="font-semibold">{percentage.toFixed(0)}%</p>
                        </div>
                        <div className="h-2 bg-muted rounded-full overflow-hidden">
                          <div className="h-full bg-accent-rose" style={{ width: `${percentage}%` }} />
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={sales.chart}>
                    ...
                    />
                    <Area type="monotone" dataKey="sales" stroke="#C2185B" fill="url(#revenueFill)" strokeWidth={2} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            )}
