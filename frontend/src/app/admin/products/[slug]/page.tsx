...
function ProductDetailContent({ slug }: { slug: string }) {
  ...
  const handleStockSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setProduct((prev) => (prev ? { ...prev, stockQuantity: updatedProduct.stockQuantity ?? updatedProduct.stock ?? stockAmount } : updatedProduct));
  ...
  if (error || !product) {
    return (
      <div className="min-h-screen bg-muted/30">
        <AdminHeader title="Product Not Found" description="The requested product could not be located">
          <Button variant="outline" size="sm" asChild>
            <Link href="/admin/products">
              <ArrowLeft className="h-4 w-4 mr-2" />
              Back to Products
        ...
        <div className="px-4 sm:px-8 py-12 max-w-2xl mx-auto">
          <Card className="border-destructive/30 bg-destructive/5 text-center p-8">
            ...
            <p className="text-muted-foreground mb-6">
              {error || `We could not find any product with the slug "${slug}". It might have been deleted or the link is incorrect.`}
            </p>
  ...
  const originalPrice = product.comparePrice ?? (product as any).compareAtPrice ?? (product.salePrice && product.salePrice < product.price ? product.price : undefined);
                ...
                <div className="relative aspect-square w-full rounded-xl overflow-hidden bg-muted/60 border border-border/40 shadow-inner flex items-center justify-center group/gallery">
                  ...
                  <div className="absolute top-3 left-3 flex flex-col gap-2 z-10">
                    ...
                    {product.isActive ? (
                      ...
                      <Badge variant="secondary" className="shadow-md bg-muted text-muted-foreground">
                        Draft / Inactive
                      </Badge>
                    )}
                  </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div>
                    ...
                    <CardDescription className="mt-1">
                      {product.category?.name ? `Category: ${product.category.name}` : 'Catalog Item'}
                    </CardDescription>
                  </div>
              ...
              <CardContent className="pt-2">
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 p-3 rounded-lg bg-muted/40 border border-border/50">
                  <div>
                    ...
                    <p className={`text-lg font-bold ${currentStock === 0 ? 'text-red-600' : currentStock <= 5 ? 'text-orange-600' : 'text-emerald-600'}`}>
                      {currentStock} units
                    </p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-lg font-semibold text-accent-rose">
                      {product.salePrice && product.salePrice < product.price ? 'On Sale' : hasDiscount ? 'Discounted' : 'Standard'}
                    </p>
                  </div>
                </div>
              </CardContent>
                  ...
                  {product.sizes && product.sizes.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {product.sizes.map((size, index) => (
                        <Badge key={index} variant="outline" className="px-3 py-1 text-sm font-medium border-border/80 bg-background">
                          {size}
                        </Badge>
                      ))}
                    </div>
                  ...
                  {product.colors && product.colors.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {product.colors.map((color, index) => {
                        const isHex = color.startsWith('#') || /^([0-9a-f]{3}|[0-9a-f]{6})$/i.test(color);
                        const hexValue = isHex ? (color.startsWith('#') ? color : `#${color}`) : null;
